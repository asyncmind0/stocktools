...
class StockBot(sleekxmpp.ClientXMPP):
    handlers = {}
...
def connect(config):
    print config.get('jabber', 'id')
    xmpp = StockBot(
        config.get('jabber', 'id'),
        config.get('jabber', 'password'))
    ...
    if xmpp.connect((config.get("jabber", "server"),
                     config.get("jabber", "port"))):
