...
class ClientTools(object):
    ...
    def get_sector_leaders2(self, sector):
        ...
        query3 = """SELECT stocks.close,stocks.volume,stocks.sym,max(stocks.date),
        analytics.name from stocks,analytics where stocks.sym = analytics.sym
        and analytics.sector ='%s' 
        group by stocks.sym"""
        cur.execute(query3, sector)
        result = cur.fetchall()
    ...
    def get_sector_leaders(self, sector):
        ...
        for sym in sector_stocks:
            ...
            WHERE stocks.sym = '%s' and analytics.sym = '%s'
            and analytics.sector = '%s' """ % (sym[0], sym[0], sector))
    ...
    def get_prices(self,stocks, column, date):
        stocks = stocks.split(',') if isinstance(stocks,(str,unicode)) else stocks
        prices = []
        if date:
            ...
            for stock in stocks:
                stock = stock.upper()
                cur = self.rpc._conn.cursor()
                query = ("""SELECT %s, max(date) FROM stocks WHERE sym='%s' and date < '%s' """
                        %(column, stock, date))
                cur.execute(query)
                result = cur.fetchall()
                cur.close()
                prices.append((stock, result[0][0] if result else result))
        else:
            for stock in stocks:
                stock = stock.upper()
                prices.append((stock,self.last_value(stock,column)))
        return prices
    ...
    def simple_moving_average(self, symbol, n=5):
        ...
        #try:
        #    cur.execute(
        #"""SELECT %s, max(date) FROM stocks WHERE sym ='%s'""" % (maN, symbol))
    ...
    def all_symbols_names_sectors(self, where):
        cur = self.rpc._conn.cursor()
        cur.execute("""SELECT sym,name,sector FROM analytics %s  ORDER BY sym""" % (where or ' '))
        result = cur.fetchall()
        cur.close()
        return result
    ...
    def get_history(self, symbols, date=None):
        cur = self.rpc._conn.cursor()
        result = {}
        for symbol in symbols:
            symbol = symbol.upper()
            cur.execute("""SELECT sym, date, open, close,
                high, low, volume FROM stocks 
                    WHERE sym = '%s' ORDER BY date""" % symbol)
            result[symbol] = cur.fetchall()
        cur.close()
        return result
...
class Portfolio(object):
    ...
    def get(self, user):
        ...
        print "got it"
        cur.execute(
            """SELECT sym,cost,date, quantity, fee
            FROM portfolios ORDER BY sym""")
        result = cur.fetchall()
        cur.close()
        return result

    def add(self, sym, cost, quantity, date, fee): 
        cur = self.rpc._conn.cursor()
        cur.execute(
            """insert into portfolios (sym, cost, date, quantity, fee)
            values ('%s', %s, %s, %s, %s)
             """ % (sym, cost, quantity, date, fee))
        self.rpc._conn.commit()
        cur.close()
        return True
